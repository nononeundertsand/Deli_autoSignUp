...
class Controller:
    ...
    def get_text_location(self,target:str):
        ...
        if isinstance(result, list) and len(result) > 0:
            try:
                for text in result[0]:
                    # print('text:', str(text[1][0]))
                    if target in str(text[1][0]):
                        print("target_text:", str(text[1][0]))
                        x1=text[0][0][0]
                        print("x1:", x1)
                        x2=text[0][2][0]
                        print("x2:", x2)
                        ...
                        print("text_location:", text_location)
                        self.temp_identify_area = text_location
                        print("self.temp_identify_area:", self.temp_identify_area)
                        break
    ...
    # 发送邮件通知
        send_email(
            subject="德力e+签到异常!!!",
            content=f"签到出现超时错误，请检查",
            sender_email="1821986277@qq.com",
            sender_password="ljgtpbygorpxbddg",
            receiver_email="1821986277@qq.com"
        )
