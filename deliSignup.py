...
class Deli:
    ...
    # 这个是启动fake_location进行虚拟定位的启动，但是因为我们一直启动所以可以省略
        ...
        try:
            def handle_in_sign_area():
                ...
                try:
                    ...
                    self.controller.get_text_location("上班打卡")
                    print("识别到目标文字：打卡",self.controller.temp_identify_area)
                    self.controller.click(*self.controller.temp_identify_area)
                    self.log.info("点击打卡按钮完毕")
                    ...
                    def handle_sign_success():
                        ...
                        try:
                            # 使用OCR定位X文字并点击
                            self.controller.get_text_location("X")
                            print("识别到目标文字：X", self.controller.temp_identify_area)
                            self.controller.click(*self.controller.temp_identify_area)
                            self.log.info("点击X按钮成功")
                        except Exception as e:
                            self.log.error(f"点击X按钮失败: {str(e)}")
                            # 如果失败，尝试原来的返回按钮
                            try:
                                self.controller.click(*self.click_pos["返回"])
                                self.log.info("点击返回按钮成功")
                            except Exception as e2:
                                self.log.error(f"点击返回按钮也失败: {str(e2)}")
                                raise
            ...
            def handle_skip():
                self.controller.get_text_location("跳过")
                self.log.info("点击跳过按钮")
                self.controller.click(*self.click_pos["跳过"])
            ...
            self.controller.device.press("home")  # 按Home键回到桌面
            ...
            self.controller.wait({
                ...
                "已在打卡范围内": handle_in_sign_area,  # 添加这个
            ...
            # 关闭应用
            self.log.info(f"关闭应用: {self.deli_package_name}")
            # 假设当前操作的 APP 包名保存在 deli_package_name
            self.controller.device.app_stop(self.deli_package_name)
            self.log.info(f"{self.deli_package_name} 已关闭")

    
            # # 退出账号
            # self.log.info("准备退出账号...")
            # # self.controller.wait("我的").wait(1).click(*self.click_pos["我的"])
            # self.controller.get_text_location("我的").click()
